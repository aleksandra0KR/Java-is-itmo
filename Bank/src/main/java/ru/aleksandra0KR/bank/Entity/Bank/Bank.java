...
public class Bank implements SubjectBank {
  ...
  public void AddUser(User user) {
      if (_users.stream().noneMatch(u -> u.getID().compareTo(user.getID()) == 0)) {
          _users.add(user);
      }
  }
  ...
  public void AddAccount(Account account) {
      if (_accounts.stream()
          .noneMatch(a -> a.getAccountId().compareTo(account.getAccountId()) == 0)) {
  ...
  /**
   * Notifies about a time change by updating the bank's calendar and calculating daily percentages
   * for all accounts.
   *
  ...
  public BigDecimal AccountDepositPercentage(BigDecimal depositMoney) {
    ...
    for (DepositMoneyGapPercentage gap : DepositPercentage) {
        if (gap.SuitedPercentage(depositMoney)) {
  ...
  public void Attach(User observer) {
      if (_observers.stream().noneMatch(u -> u.getID().compareTo(observer.getID()) == 0)) {
  ...
  /**
   * Opens a new debit account for a user with specified initial money and duration in the past
   * years.
   *
  ...
  public DebitAccount openDebitAccount(User user, BigDecimal money, int years) {
    ...
    DebitAccount debitAccount = new DebitAccount(money, BankCalendar, closeDate, user, this,
        DebitPercentage);
  ...
  /**
   * Opens a new credit account for a user with specified initial money, duration in years, credit
  ...
  public CreditAccount openCreditAccount(User user, BigDecimal money, int years) {
    LocalDate closeDate = BankCalendar;
    closeDate = closeDate.plusYears(years);
    CreditAccount creditAccount = new CreditAccount(money, BankCalendar, closeDate, user, this,
        CreditLimit, CreditCommission);
    this.AddAccount(creditAccount);
    return creditAccount;
  }

  /**
   * Opens a new deposit account for a user with specified initial money, duration in years, and
   * calculates the appropriate deposit percentage based on deposited amount.
   *
   * @param user  the user opening the account
   * @param money initial amount of money in the account
   * @param years duration in years until account closure
  ...
  public DepositAccount openDepositAccount(User user, BigDecimal money, int years) {
    LocalDate closeDate = BankCalendar;
    closeDate = closeDate.plusYears(years);
    DepositAccount depositAccount = new DepositAccount(money, BankCalendar, closeDate, user, this,
        AccountDepositPercentage(money));
